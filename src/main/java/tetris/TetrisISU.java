...
public class TetrisISU extends JFrame implements KeyListener {
    ...
    public static void main(String[] args) {
        final TetrisISU frame = new TetrisISU();
    ...
    public static void startGame() {
        //make the gameRunning boolean set to true
        gameRunning = true;
        ...
        frame.addWindowListener(new WindowAdapter() {
            public void windowClosing(WindowEvent event) {
                //if the user presses the x button, set the gameRunning boolean to false and dispose the frame
                //****!!!*****THIS MEANS THE MUSIC WONT STOP UNTIL IT FINISHES PLAYING THE FILE****!!!*****
                gameRunning = false;
                frame.dispose();
            }
        });
    ...
    /**
     *
     * @param evt when the keylistener picks up a keypress it calls this method
     * and passes it an object with all the information about the keypress
     */
    @Override
    ...
    /**
     *
     * @param x the argument passed indicates whether the piece should be moved
     * to left of right with -1 being left and 1 being right
     *
     */
    public void moveLR(int x) {
        for (Point p : tetris.Shapes.TetrisShapes[currentPiece][rotation]) {
    ...
    public boolean collision(int x, int y, int rotationPos) {
        for (Point p : tetris.Shapes.TetrisShapes[currentPiece][rotationPos]) {
    ...
    public void endGame() {
        ...
        //try catch for catching any errors while sending to the google sheet
        try {
            //check to see if the score in the game is greater than the current high score, if it is, then set your score to be the current score (the new highscore)
            if (arcadeGUI.getScore() < score) {
                arcadeGUI.setScore(score);
            }
        } catch (IOException ex) {
            Logger.getLogger(TetrisISU.class.getName()).log(Level.SEVERE, null, ex);
        }
